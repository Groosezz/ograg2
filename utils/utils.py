...
def _create_chat_llm(
    ...
    api_type = (api_type or "azure").lower()

    if api_type == "azure":
        if resource_endpoint and api_key:
    ...
    if api_type == "openai":
        ...
        if resource_endpoint:
            chat_kwargs["base_url"] = resource_endpoint
    ...
    raise ValueError(f"Unsupported model api_type: {api_type}")


def _create_embedding_model(embedding_config: Dict[str, Any]):
    emb_api_type = (embedding_config.get("api_type") or "azure").lower()
    emb_api_key = embedding_config.get("api_key") or os.getenv("OPENAI_API_KEY")
    emb_endpoint = embedding_config.get("api_base") or os.getenv(
        embedding_config.get("api_base_env_var", "")
    )
    emb_model_name = embedding_config.get("deployment_name") or os.getenv(
        embedding_config.get("deployment_name_env_var", "")
    )
    emb_api_version = embedding_config.get("api_version") or os.getenv(
        embedding_config.get("api_version_env_var", "")
    )

    if emb_api_type == "azure":
        if emb_api_key:
    ...
    if emb_api_type == "openai":
        ...
        if emb_endpoint:
            emb_kwargs["openai_api_base"] = emb_endpoint
...
def load_llm_and_embeds(model_config: Dict[str, Any], embedding_config: Dict[str, Any]) -> Tuple[ChatOpenAI, Embeddings]:
    ...
    embedding_llm = _create_embedding_model(embedding_config)
